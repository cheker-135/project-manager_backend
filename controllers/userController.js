...
const registerUser = asyncHandler(async (req, res) => {
  ...
  try {
    await User.create({
      ...
      email,
      password,
      emailConfirmed: true, // Set emailConfirmed to true for the user
      projectsThemes: {},
    });
...
export const updateUserAgentStatus = async (req, res) => {
  ...
  try {
    ...
    // Update isAgent status
    user.isAgent = isAgent;
    await user.save();

    res.status(200).json({ message: 'User isAgent status updated successfully', user });
  } catch (error) {
...
const getNotifications = asyncHandler(async (req, res) => {
  const defaultUserId = req.user._id;
  console.log("defaultUserId:" ,defaultUserId);
  const notifications = await Notification.find({
  //  recipient: req.user._id,
   recipient: defaultUserId,
  })
    .sort({ createdAt: -1 })
    .populate('project')
    .populate('task')
